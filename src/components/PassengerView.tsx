...
interface PassengerViewProps {
  ...
  onCancelTrip: (tripId: string, reason?: string, cancelledBy?: 'passenger' | 'driver') => Promise<void>;
...
export const PassengerView: React.FC<PassengerViewProps> = ({
  ...
  const [cancelReason, setCancelReason] = useState<string>('Changed my mind / plan');
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-7 space-y-4">
          <div className="bg-white dark:bg-slate-900 rounded-3xl p-5 border border-slate-200 dark:border-slate-800 shadow-sm space-y-4">
            ...
            {/* Pinning Banner Notification */}
            {mapSelectionMode && (
              <div className="p-3 bg-amber-50 dark:bg-amber-950/60 border border-amber-300 dark:border-amber-700 rounded-2xl flex items-center justify-between gap-2 animate-in fade-in">
                <div className="flex items-center space-x-2 text-amber-900 dark:text-amber-200 text-xs font-bold">
                  ...
                  <span>
                    {mapSelectionMode === 'pickup'
                      ? (lang === 'am' ? 'መነሻ ቦታ ለመምረጥ ካርታው ላይ ይጫኑ' : 'Click anywhere on map to set Pickup location')
                      : (lang === 'am' ? 'መድረሻ ቦታ ለመምረጥ ካርታው ላይ ይጫኑ' : 'Click anywhere on map to set Destination')}
                  </span>
                </div>
                <button
                  type="button"
                  onClick={() => setMapSelectionMode(null)}
                  className="px-3 py-1 rounded-xl bg-white dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 text-xs font-bold border border-slate-200 dark:border-slate-700 cursor-pointer shadow-xs"
                >
                  {lang === 'am' ? 'ተመለስ / ሰርዝ' : '← Done / Cancel'}
                </button>
              </div>
            )}
        ...
        <div className="lg:col-span-5 space-y-4">
          ...
          {activeTrip && activeTrip.status === 'ringing' && (
            <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 border-2 border-emerald-400 shadow-xl space-y-5">
              ...
              {/* Actions: Change details / Cancel Call */}
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 pt-1">
                <button
                  type="button"
                  onClick={() => onCancelTrip(activeTrip.id, 'Customer changed pickup or destination details', 'passenger')}
                  className="py-3 px-3 rounded-2xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 font-bold text-xs transition-colors flex items-center justify-center space-x-1.5 cursor-pointer"
                >
                  <RotateCcw className="w-3.5 h-3.5" />
                  <span>{lang === 'am' ? '← መረጃ ቀይር (ተመለስ)' : '← Change Details'}</span>
                </button>

                <button
                  type="button"
                  onClick={() => onCancelTrip(activeTrip.id, 'Customer cancelled contract request', 'passenger')}
                  className="py-3 px-3 rounded-2xl border border-rose-200 dark:border-rose-900/50 bg-rose-50 dark:bg-rose-950/30 hover:bg-rose-100 text-rose-700 dark:text-rose-400 font-bold text-xs transition-colors flex items-center justify-center space-x-1.5 cursor-pointer"
                >
                  <XCircle className="w-3.5 h-3.5" />
                  <span>{lang === 'am' ? 'ጥሪውን ሰርዝ' : 'Cancel Request'}</span>
                </button>
              </div>
            </div>
          )}
          ...
          {activeTrip && activeTrip.status === 'expired' && (
            <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 border-2 border-rose-300 dark:border-rose-900 shadow-xl text-center space-y-5 animate-in fade-in">
              ...
              <div className="flex flex-col sm:flex-row gap-2 pt-2">
                <button
                  type="button"
                  onClick={() => onCancelTrip(activeTrip.id, 'Retry search after timeout', 'passenger')}
                ...
                >
                  <RotateCcw className="w-3.5 h-3.5" />
                  <span>{lang === 'am' ? '← ወደ መነሻ ተመለስና ይሞክሩ' : '← Back to Booking / Try Again'}</span>
                </button>
                <a
          ...
          {activeTrip && ['accepted', 'en_route', 'arrived'].includes(activeTrip.status) && (
            <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 border-2 border-emerald-500 shadow-xl space-y-5 animate-in zoom-in-95">
              ...
              {/* Trip Actions: Customer Cancel & Complete Ride */}
              <div className="flex flex-col sm:flex-row items-center gap-2.5 pt-1">
                <button
                  type="button"
                  id="btn-customer-cancel-ride"
                  onClick={() => setIsCancelModalOpen(true)}
                  className="w-full sm:w-auto px-4 py-3.5 rounded-2xl border border-rose-300 dark:border-rose-800/70 bg-rose-50 dark:bg-rose-950/40 hover:bg-rose-100 text-rose-700 dark:text-rose-300 font-bold text-xs transition-colors flex items-center justify-center space-x-1.5 cursor-pointer shrink-0"
                >
                  <XCircle className="w-4 h-4 text-rose-500" />
                  <span>{t(lang, 'cancelRide')}</span>
                </button>
                ...
                <button
                  ...
                  className="flex-1 w-full py-3.5 rounded-2xl bg-emerald-500 hover:bg-emerald-600 text-white font-bold text-xs sm:text-sm shadow-md shadow-emerald-500/20 transition-all cursor-pointer font-['Outfit'] flex items-center justify-center space-x-2"
                >
                  <CheckCircle2 className="w-4 h-4" />
                  <span>{t(lang, 'completeTrip')}</span>
                </button>
              </div>
            </div>
          )}
            ...
            <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 sm:p-7 border border-slate-200 dark:border-slate-800 shadow-sm space-y-5">
              ...
              <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-3">
                ...
                <button
                  type="button"
                  onClick={() => {
                    if (districtLandmarks[0]) {
                      setPickupLandmarkId(districtLandmarks[0].id);
                      setPickupAddress(districtLandmarks[0].name);
                      setPickupCoords({ lat: districtLandmarks[0].lat, lng: districtLandmarks[0].lng });
                    }
                    if (districtLandmarks[1]) {
                      setDropoffLandmarkId(districtLandmarks[1].id);
                      setDropoffAddress(districtLandmarks[1].name);
                      setDropoffCoords({ lat: districtLandmarks[1].lat, lng: districtLandmarks[1].lng });
                    }
                    setPassengerCount(1);
                    setHasLuggage(false);
                    setNotes('');
                  }}
                  className="px-2.5 py-1.5 rounded-xl text-[11px] font-bold text-slate-500 hover:text-slate-800 dark:text-slate-400 dark:hover:text-white bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 flex items-center gap-1.5 cursor-pointer transition-colors shadow-xs"
                  title="Reset form fields"
                >
                  <RotateCcw className="w-3 h-3" />
                  <span>{t(lang, 'resetForm')}</span>
                </button>
              </div>
          ...
          <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 sm:p-7 max-w-md w-full border border-slate-200 dark:border-slate-800 shadow-2xl space-y-5 animate-in zoom-in-95">
              ...
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <h3 className="font-bold text-slate-900 dark:text-white text-base font-['Outfit']">
                    {t(lang, 'cancelRideConfirmTitle')}
                  </h3>
                  <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
                    {t(lang, 'cancelRideConfirmDesc')}
                  </p>
                </div>
              </div>
              <button
            ...
            <div className="space-y-2">
              <label className="text-xs font-bold text-slate-700 dark:text-slate-300 block">
                {t(lang, 'cancellationReasonLabel')}
              </label>

              <div className="space-y-2 max-h-48 overflow-y-auto pr-1">
                {[
                  { key: 'changed_mind', label: t(lang, 'reasonChangedMind') },
                  { key: 'found_other', label: t(lang, 'reasonFoundOther') },
                  { key: 'wrong_location', label: t(lang, 'reasonWrongLocation') },
                  { key: 'driver_delay', label: t(lang, 'reasonDriverDelay') },
                  { key: 'price_disagreement', label: t(lang, 'reasonPriceDisagreement') },
                  { key: 'other', label: t(lang, 'reasonOther') },
                ].map((item) => (
                  <label
                    key={item.key}
                  ...
                  >
                    <input
                      ...
                      name="cancel_reason"
                      value={item.label}
                      checked={cancelReason === item.label}
                      onChange={() => setCancelReason(item.label)}
              ...
              {cancelReason === t(lang, 'reasonOther') && (
                <input
                  type="text"
                  value={customCancelReason}
                  onChange={(e) => setCustomCancelReason(e.target.value)}
                  placeholder={lang === 'am' ? 'ምክንያትዎን እዚህ ይጻፉ...' : 'Please specify other reason...'}
                  className="w-full mt-2 px-3.5 py-2 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-white text-xs outline-hidden focus:ring-2 focus:ring-rose-500"
                />
              )}
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-2.5 pt-2">
              ...
              <button
                ...
                onClick={async () => {
                  ...
                  const finalReason = cancelReason === t(lang, 'reasonOther') && customCancelReason.trim()
                    ? customCancelReason.trim()
                    : cancelReason;
                  try {
                    await onCancelTrip(activeTrip.id, finalReason, 'passenger');
              ...
              >
                <XCircle className="w-3.5 h-3.5" />
                <span>{isCancellingTrip ? (lang === 'am' ? 'እየተሰረዘ ነው...' : 'Cancelling...') : t(lang, 'confirmCancelBtn')}</span>
              </button>
            </div>
